import $ from 'jquery';

let data = (localStorage.getItem('todoList'))
  ? JSON.parse(localStorage.getItem('todoList'))
  : {
    todo: [],
    completed: []
  };

console.dir(data);

/**
 * Class Declaration for Todo List App
 * @param (User) Name of the Person who is logged in the application
 * @return { Void }
 */
class TodoListApp {

  /**
   * Initialize todo app handler
   * @param null
   * @return { Void }
   */
  init() {
    this.renderTodoList();
    $('#add').on('click', () => {
      let value = $('#item').val();
      if (value) {
        this.addItem(value); //Run this method with the passed value
      }
    });
    $('#item').on('keydown', (event) => {
      const key = event.keyCode || event.which;
      let value = $('#item').val();
      if (value && key === 13) {
        this.addItem(value); //Run this method with the passed value
      }
    });
  }

  /**
   * Add Item Method Handler. This method adds the value to the list of tasks.
   * @param value
   * @return { Void }
   */
  addItem(value) {
    this.addItemToDOM(value);
    // Clear the value
    $('#item').val('');

    // Add the value to the todo list
    data
      .todo
      .push(value);

    this.dataObjectUpdated();
  }

  /**
   * Renders the todo list with the required data, 
   * Only if the data existes either in the todo or completed list of tasks
   * @param null
   * @return { Void }
   */
  renderTodoList() {
    if (!data.todo.length && !data.completed.length) {
      return;
    }
    for (let i in data.todo) {
      let value = data.todo[i];
      this.addItemToDOM(value);
    }
    for (let i in data.completed) {
      let value = data.completed[i];
      this.addItemToDOM(value, true);
    }
  }

  /**
   * Set the value to the localstorage
   * @param null
   * @return { Void }
   */
  dataObjectUpdated() {
    localStorage.setItem('todoList', JSON.stringify(data));
  }

  /**
   * Remove the item from the list
   * @param self
   * @return { Void }
   */
  removeItem(self) {
    let _self = self;
    const $item = $(event.target).closest('li');
    const $parent = $item.closest('ul');
    const id = $parent.attr('id');
    const value = $item.text();

    if (id === 'todo') {
      data
        .todo
        .splice(data.todo.indexOf(value.trim('')), 1);
    } else {
      data
        .completed
        .splice(data.completed.indexOf(value.trim('')), 1);
    } //Check for todo tasks vs completed tasks

    _self.dataObjectUpdated();
   $item.remove();
  }

  /**
   * Adds data to the respective container object based on whether it is completed 
   * or yet to be done.
   * @param self
   * @return { Void }
   */
  completeItem(self) {
    let _self = self;
    const $item = $(event.target).closest('li');
    const $parent = $item.closest('ul');
    const id = $parent.attr('id');
    const value = $item.text();

    if (id === 'todo') {
      data
        .todo
        .splice(data.todo.indexOf(value.trim('')), 1);
      data
        .completed
        .push(value);
    } else {
      data
        .completed
        .splice(data.completed.indexOf(value.trim('')), 1);
      data
        .todo
        .push(value);
    }
    self.dataObjectUpdated();

    // Check if the item should be added to the completed list or re-added to the
    // todo list
    const target = (id === 'todo')
      ? document.getElementById('completed')
      : document.getElementById('todo');

    $item.remove();
    $('#' + target.id).append($item);
  }

  /**
   * Add a new item to the TODO List
   * @param text, completed
   * @return { Void }
   */
  addItemToDOM(text, completed) {
    const list = (completed)
      ? document.getElementById('completed')
      : document.getElementById('todo');
    const item = document.createElement('li');
    const buttons = document.createElement('div');
    const remove = document.createElement('button');
    const complete = document.createElement('button');
    const removeSVG = `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 22 22" style="enable-background:new 0 0 22 22;" xml:space="preserve"> <rect class="noFill" width="22" height="22"/> <g> <g> <path class="fill" d="M16.1,3.6h-1.9V3.3c0-1.3-1-2.3-2.3-2.3h-1.7C8.9,1,7.8,2,7.8,3.3v0.2H5.9c-1.3,0-2.3,1-2.3,2.3v1.3c0,0.5,0.4,0.9,0.9,1v10.5c0,1.3,1,2.3,2.3,2.3h8.5c1.3,0,2.3-1,2.3-2.3V8.2c0.5-0.1,0.9-0.5,0.9-1V5.9C18.4,4.6,17.4,3.6,16.1,3.6z M9.1,3.3c0-0.6,0.5-1.1,1.1-1.1h1.7c0.6,0,1.1,0.5,1.1,1.1v0.2H9.1V3.3z M16.3,18.7c0,0.6-0.5,1.1-1.1,1.1H6.7c-0.6,0-1.1-0.5-1.1-1.1V8.2h10.6V18.7z M17.2,7H4.8V5.9c0-0.6,0.5-1.1,1.1-1.1h10.2c0.6,0,1.1,0.5,1.1,1.1V7z"/> </g> <g> <g> <path class="fill" d="M11,18c-0.4,0-0.6-0.3-0.6-0.6v-6.8c0-0.4,0.3-0.6,0.6-0.6s0.6,0.3,0.6,0.6v6.8C11.6,17.7,11.4,18,11,18z"/> </g> <g> <path class="fill" d="M8,18c-0.4,0-0.6-0.3-0.6-0.6v-6.8c0-0.4,0.3-0.6,0.6-0.6c0.4,0,0.6,0.3,0.6,0.6v6.8C8.7,17.7,8.4,18,8,18z"/> </g> <g> <path class="fill" d="M14,18c-0.4,0-0.6-0.3-0.6-0.6v-6.8c0-0.4,0.3-0.6,0.6-0.6c0.4,0,0.6,0.3,0.6,0.6v6.8C14.6,17.7,14.3,18,14,18z"/> </g> </g> </g></svg>`;

    const completeSVG = `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 22 22" style="enable-background:new 0 0 22 22;" xml:space="preserve"> <rect y="0" class="noFill" width="22" height="22"/> <g> <path class="fill" d="M9.7,14.4L9.7,14.4c-0.2,0-0.4-0.1-0.5-0.2l-2.7-2.7c-0.3-0.3-0.3-0.8,0-1.1s0.8-0.3,1.1,0l2.1,2.1l4.8-4.8c0.3-0.3,0.8-0.3,1.1,0s0.3,0.8,0,1.1l-5.3,5.3C10.1,14.3,9.9,14.4,9.7,14.4z"/> </g></svg>`;

    item.innerText = text;
    buttons
      .classList
      .add('buttons');
    remove
      .classList
      .add('remove');
    remove.innerHTML = removeSVG;

    // Add click event for removing the item
    remove.addEventListener('click', () => {
      const self = this;
      this.removeItem(self);
    });
    complete
      .classList
      .add('complete');
    complete.innerHTML = completeSVG;

    // Add click event for completing the item
    complete.addEventListener('click', () => {
      const self = this;
      this.completeItem(self);
    });

    buttons.appendChild(remove);
    buttons.appendChild(complete);
    item.appendChild(buttons);

    $('#' + list.id).append(item);
  }

}

const app = new TodoListApp();
app.init();